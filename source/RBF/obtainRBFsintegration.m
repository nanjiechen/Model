%% 1. Construct object (New verison Radialbasis.m)

%             numofloop=10;
%             densfactor_= 2; % for simpson
%             Np=11;
            xp = linspace(-3, 3, Np);
%             yp = linspace(-3, 3, Np);
...
%         RBFopts.kerneltype = 6; 
    ...
    % 2. Construct Object
        R = Radialbasis(gridnodes,lowerbounds,upperbounds,RBFopts,POLYopts,xp); % Notice that we add a parameter 'xp' in the last position

%% New tech
 
         load('weihgt&roots.mat'); % the data file name may vary!
         
         % Notice that you should already constructed an object before
             R.roots_1d=roots;
             R.weights_1d=weights;

             quabak=R.Baking; % for example, quabak is an 27*27 matrix which's each element is the integration value of the RBF(centered at that meshgrid_knots)*Gaussian density 
...
% 2. one quadrant
    for i = 1:centerindex-1 % position i
      ...
      % 2.1. Evaluate
        ys = Radialbasis.kernelmatrix(center,[XX,YY],RBFopts);
        yg=ys.*K2; 
...
% 4. Combine them into a square matrix
